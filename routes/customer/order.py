...
async def place_order(
    ...
    try:
        ...
        response_data = {
            "success": True,
            "message": "Order placed successfully" if not requires_payment else "Order created. Please complete payment.",
            "data": {
                "order_id": order.id,
        ...
        if not requires_payment:
            ...
            try:
                await send_notification(
                    current_user.id, 
                    "Order Placed", 
                    f"Your COD order has been placed with ID: {order.id}"
                )
                await send_notification(
                    vendor.user_id, 
                    "New Order Received", 
                    f"A new COD order has been placed by {current_user.name}"
                )
            except Exception as e:
