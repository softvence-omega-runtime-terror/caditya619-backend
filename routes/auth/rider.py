...
async def signup(
    ...
    if not await verify_otp(phone, otp, purpose='rider_signup'):
        raise HTTPException(status_code=400, detail="OTP not verified.")

    async with in_transaction() as connection:
        user = await User.get_or_none(phone=phone)
        
        if user and user.is_rider:
            raise HTTPException(status_code=400, detail="Already registered as rider.")

        if not user:
            user = await User.create(phone=phone, is_rider = True, name=name, using_db=connection)
